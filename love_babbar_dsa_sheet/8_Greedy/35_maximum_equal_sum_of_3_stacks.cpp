...
int maxSum(int stack1[], int stack2[], int stack3[], int n1, int n2, int n3)
{
	...
	while (1) {
		...
		// Finding the stack with maximum sum and removing its top element from sum.

		if (sum1 >= sum2 && sum1 >= sum3) // if sum1 is maximum
			sum1 -= stack1[top1++];

		else if (sum2 >= sum1 && sum2 >= sum3) // if sum2 is maximum
			sum2 -= stack2[top2++];

		else if (sum3 >= sum2 && sum3 >= sum1) // if sum3 is maximum
			sum3 -= stack3[top3++];
	}
}
