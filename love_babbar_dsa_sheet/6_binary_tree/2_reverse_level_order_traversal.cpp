...
void level_order_traversal(Node* root) {
    ...
    // for every level call print current level function to print all nodes of current level, from last to first
    for(int i=height; i>=0; i--) {
        printCurrLevel(root, i);
    }
}

// // Approach - 2 Using queue + stack DS - TC O(N) SC O(N)
void level_order_traversal(Node* root) {
    // Base Case
    if(!root) {
        cout << "Empty tree ";
    ...
    // Create an empty queue and stack for reverse level order traversal
    queue<Node*> q;
    stack<Node*> s;
    // Enqueue Root
    q.push(root);
    ...
    while(!q.empty()) {
        // take front of queue in temp and remove it from queue
        Node* temp = q.front();
        q.pop();

        // intstead of printing it here we will add in stack to get reverse order
        s.push(temp);

        /* Enqueue right child first then left to maintain the sequence of ans */
    ...
    while(!s.empty()) {
        // print top of and stack and remove it from stack
        Node* temp = s.top();
        cout << temp->data << " ";
        s.pop();
    }
