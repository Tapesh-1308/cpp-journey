...
void fillMap(Node* root, int hd, int l, map<int, pair<int, int> >& m) {
    ...
    // map {curr node, curr level} with hd only if, this is the first node of curr hd or curr level is lower than the level already present in map 
...
void top_view(Node* root) {
    ...
    while(!q.empty()){
        ...
        // map frontNode with key hd, only if it is not present   
    ...
    // print top view which is contained me map 
    for(auto i: topNode)
        cout << i.second << " ";
